...
func (c *PodGroupSetController) ProcessNewPodGroupSet(ctx context.Context, pgs *v1alpha1.PodGroupSet) error {
	...
	for _, level := range slices.Backward(topology.Levels) {
		for _, topologyNode := range topology.Nodes[level] {
			...
			placements, weight, err := c.schedulePodGroupSet(ctx, pgs, topologyNode.Nodes, c.podPipeline)
			if err != nil {
...
func (c *PodGroupSetController) schedulePodGroupSet(ctx context.Context, pgs *v1alpha1.PodGroupSet, nodes []corev1.Node, podPipeline lib.Pipeline[pods.PodPipelineRequest]) (map[string]string, float64, error) {
...
func (c *PodGroupSetController) initPodPipeline(ctx context.Context) error {
	...
	podPipeline, err := lib.NewPipeline(ctx, c.Client, pipeline.Name, SupportedSteps, pipeline.Spec.Steps, c.Monitor)
	if err != nil {
		return fmt.Errorf("failed to create pod pipeline: %w", err)
	}

	c.podPipeline = podPipeline
	return nil
}
